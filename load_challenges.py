...
from pointcalc import pointcalc, perimeter_distances, distance_dict
...
forbidden_zones = [113, 114, 115, 116, 117, 118, 124, 161, 162, 163, 164, 184]
...
class RawChallenge:
    ...
    def challenge(self, zoned: bool, id: int, specific: bool, current_zone: int, zkaff: bool = False) -> Challenge:
        ...
        points = pointcalc(self.kaffness, self.grade, self.points, self.walking_minutes, self.stationary_minutes, self.ppr, reps, zone, bias, self.fixed, current_zone, self.zoneable and zoned, self.dead_end, self.station_distance, self.time_to_hb, self.departures)
...
for i in range(len(da_new_kaff_sheet)):
    ...
    bias = float(bias)
    place = str(place)
    if place.lower() in [shop.lower() for shop in GLOBAL_SHOPS]:
        continue

    discount = pointcalc(kaffness=kaffness, grade=grade, zone=zone, bias=1, reps=0, ppr=0, challenge_points=0, walking_minutes=0, stationary_minutes=0, fixed=True, current_zone=zone, zoneable_and_zoned=False, dead_end=False, station_distance=0, time_to_hb=0, departures=None)
...
def specific_challenge_generate(index, current_zone) -> Challenge:
    return specific_challenges[index].challenge(zoned=True, id=index, specific=True, current_zone=current_zone)


def zurich_challenge_generate(index, current_zone) -> Challenge:
    return zurich_challenges[index].challenge(zoned=True, id=index, specific=True, current_zone=current_zone, zkaff=True)


def unspecific_challenge_generate(index, current_zone):
